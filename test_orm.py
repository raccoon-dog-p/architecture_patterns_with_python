...
def test_주문_라인_매퍼_Read_확인(session):
    session.execute(
...
def test_주문_라인_매퍼_Create_확인(session):
...
def test_배치_매퍼_Read_확인(session):
    session.execute(
...
def test_배치_매퍼_Create_확인(session):
...
def test_배치에_주문_라인_할당_되는지_확인(session):
    batch = model.Batch("batch1", "sku1", 100, eta=None)
...
def test_배치에_할당된_라인_Read_확인(session):
    session.execute(
